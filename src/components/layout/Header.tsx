...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Navigation */}
      <div className={`lg:hidden transition-all duration-400 overflow-hidden ${isMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
          ...
          <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
            ...
            <div className="pt-4 border-t border-gold/10">
              {user ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 px-2">
                    <div className="bg-cream rounded-xl p-2 text-center">
                      <p className="text-xs text-gray-500">Niveau</p>
                      <p className="text-xs font-medium text-navy truncate">{user.niveau || 'Non défini'}</p>
                    </div>
                    <div className="bg-cream rounded-xl p-2 text-center">
                      <p className="text-xs text-gray-500">Filière</p>
                      <p className="text-xs font-medium text-navy truncate">{user.filiere || 'Non défini'}</p>
                    </div>
                  </div>
                  <button
                    onClick={handleLogout}
                ...
                // Mobile - Connexion et Inscription
                <div className="space-y-3">
                  <Link
                    to="/connexion"
                    className="block text-center text-navy/70 hover:text-gold font-medium transition-colors py-2"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    Connexion
                  </Link>
